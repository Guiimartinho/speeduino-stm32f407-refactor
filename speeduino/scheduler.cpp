...
{
  // FASE OPT-1: Switch optimization for better branch prediction (10-15% faster)
  switch(schedule.Status)
  {
    case PENDING: //Check to see if this schedule is turn on
...
{
  // FASE OPT-2: Switch + micros() caching optimization (15-20% faster)
  // Cache micros() call - expensive system call, ~50-100 cycles
  ...
  switch(schedule.Status)
  {
    case PENDING: //Check to see if this schedule is turn on
      ...
      schedule.Status = RUNNING; //Set the status to be in progress (ie The start callback has been called, but not the end callback)
      schedule.startTime = currentMicros;
    ...
    case RUNNING:
      ...
      currentStatus.actualDwell = DWELL_AVERAGE( (currentMicros - schedule.startTime) );
