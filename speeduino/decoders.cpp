...
void triggerPri_RoverMEMS()
{
  ...
  if ( curGap >= triggerFilterTime ) //Pulses should never be less than triggerFilterTime, so if they are it means a false trigger. (A 36-1 wheel at 8000pm will have triggers approx. every 200uS)
  {
    ...
    {   
      ...
      {
        // Check all RoverMEMS patterns using data-driven approach
        if(!checkAndConfigureRoverMEMSPattern() && (toothCurrentCount > triggerActualTeeth+1)) // no patterns match after a rotation when we only need 32 teeth to match, we've lost sync
        {
          currentStatus.hasSync = false;
          BIT_CLEAR(currentStatus.status3, BIT_STATUS3_HALFSYNC);
          currentStatus.syncLossCounter++;              
